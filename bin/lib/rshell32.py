...
# BEB: Loboris' python used UNIX 1970 epoch ...
TIME_OFFSET = 0
...
def find_macthing_files(match):
    """Finds all of the files with match (used for completion)."""
...
def rsync(src_dir, dst_dir, mirror, dry_run, print_func, recursed):
    ...
    for src_basename in to_add:  # Name in source but absent from destination
        if src_basename.startswith('.'):
            # BEB
            print_func("{} starts with a dot - skipping".format(src_basename))
            continue
...
class DeviceSerial(Device):
    ...
    def __init__(self, port, baud, wait):
        ...
        try:
            ...
            # BEB: don't abort, pass issue on instead ...
            # sys.exit(1)
            raise
...
def real_main():
    ...
    if args.port:
        try:
            ...
            print(err)
        except PyboardError as err2:
            # BEB do not throw the towel if the USB port does not exist ...
            print(err2)
    else:
        autoscan()
    autoconnect()
